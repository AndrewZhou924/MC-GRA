...
parser.add_argument('--nlayers', type=int, default=2, help="number of layers in GCN.")
...
parser.add_argument('--max_eval', type=int, help="max evaluation times for searching", default=100)
parser.add_argument('--log_name', type=str, help="file name to save the result", default="result.txt")
...
parser.add_argument("--measure", type=str, default="HSIC", choices=["HSIC", "MSELoss", "KL", "KDE", "CKA", "DP"])
parser.add_argument("--measure2", type=str, default="HSIC", choices=["HSIC", "MSELoss", "KL", "KDE", "CKA", "DP"])
parser.add_argument("--nofeature", action='store_true')

parser.add_argument('--weight_aux', type=float, default=0, help="the weight of auxiliary loss")
parser.add_argument('--weight_sup', type=float, default=1, help="the weight of supervised loss")
...
if args.arch == "gcn":
    ...
    if args.defense:
        victim_model.load_state_dict(torch.load("./defense/"+args.dataset+'_gcn_2.pt', map_location = device))
        victim_model = victim_model.to(device)
    else:
        victim_model = victim_model.to(device)
        victim_model.fit(features, adj, labels, idx_train, idx_val)

    embedding = embedding_GCN(nfeat=features.shape[1], nhid=16, nlayer=args.nlayers, device=device)
...
if args.arch == 'sage':
    ...
    if args.defense:
        victim_model.load_state_dict(torch.load("./defense/"+args.dataset+'_sage_2.pt'))
        victim_model = victim_model.to(device)
    else:
        victim_model = victim_model.to(device)
        victim_model.fit(features, adj, labels, idx_train, idx_val)

    embedding = embedding_graphsage(nfeat=features.shape[1], nhid=16, nlayer=args.nlayers, device=device)
    embedding.load_state_dict(transfer_state_dict(victim_model.state_dict(), embedding.state_dict()))
    print(victim_model.state_dict().keys())
    embedding.gc = deepcopy(victim_model.gc)
...
if args.arch == 'gat':
    ...
    if args.defense:
        victim_model.load_state_dict(torch.load("./defense/"+args.dataset+'_gat_2.pt'))
        victim_model = victim_model.to(device)
    else:
        victim_model = victim_model.to(device)
...
baseline_model=baseline.PGDAttack(model=victim_model, embedding=embedding, nnodes=adj.shape[0], loss_type="CE", device=device)
...
def objective(arg):
    ...
    index_delete_train=index_train[:int(len(real_edge_train)-2*np.sum(real_edge_train))]

    idx=np.arange(adj.shape[0])
    real_edge_all=ori_adj[idx, :][:, idx].reshape(-1)
    index_all=np.where(real_edge_all==0)[0]
    index_delete_all=index_all[:int(len(real_edge_all)-2*np.sum(real_edge_all))]
...
def gaussian_reparameterize_attack(arg):
    ...
    gaussian_model=gaussian_parameterized.PGDAttack(H_A=H_A2, Y_A=Y_A, model=victim_model, embedding=embedding, nnodes=adj.shape[0], loss_type="CE", device=device)
...
def gcn_reparameterize_attack(arg):
    ...
    gcn_model=gcn_parameterized.PGDAttack(H_A=H_A2, Y_A=Y_A, features=features, model=victim_model, embedding=embedding, nnodes=adj.shape[0], loss_type="CE", device=device)
