...
def reproduce(trial): 
    param = {}
    param['plain_acc'] = 0.6461 # plain_acc_maps[args.dataset]
    ...
    if args.MI_type == 'KDE':
        ...
        if args.dataset == 'enzyme':
            param['aug_pe'] = trial.suggest_float('aug_pe', low=0.4, high=0.9, step=0.1)
            for layer_id in range(args.nlayer+1): # n gconv + 1 Linear
                param_name = 'layer-{}'.format(layer_id)
                if layer_id == 0:
                    param[param_name] = trial.suggest_float(param_name, low=20, high=40, step=0.1)
                else:
                    param[param_name] = trial.suggest_float(param_name, low=6, high=15, step=0.1)

            for layer_id in range(args.nlayer):
                param_inter = 'layer_inter-{}'.format(layer_id)
                param[param_inter] = trial.suggest_float(param_inter, low=0, high=10, step=0.1)

        if args.dataset=='ogb_arxiv':
            param['aug_pe'] = trial.suggest_float('aug_pe', low=0, high=0.8, step=0.1)
            for layer_id in range(args.nlayer+1): # n gconv + 1 Linear
                param_name = 'layer-{}'.format(layer_id)
                if layer_id == 0:
                    param[param_name] = trial.suggest_float(param_name, low=0, high=20, step=0.1)
                else:
                    param[param_name] = trial.suggest_float(param_name, low=0, high=10, step=0.1)
...
with open(study_name+'_reproduce.txt', 'a') as f:
    ...
    if args.MI_type != 'DP':
        if args.dataset == 'enzyme':
            for trail in study.trials: # best_trials trials
                if trail.values is None: continue
