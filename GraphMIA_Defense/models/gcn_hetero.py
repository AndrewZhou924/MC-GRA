...
class GCN(nn.Module):
    ...
    def _train_with_val(self, labels, idx_train, idx_val, train_iters, verbose):
        ...
        for i in range(train_iters):
            ...
            # * add Gaussian noise
            for layer in self.gc:
                layer.weight.grad = layer.weight.grad + (torch.randn_like(layer.weight.grad) * stddev + mean)
            self.linear1.weight.grad = self.linear1.weight.grad + (torch.randn_like(self.linear1.weight.grad) * stddev + mean)
    ...
    def _train_with_MI_constrain(
        ...
        homo_adj, hetero_adj = homo_hetero_edge_extractor(self.origin_adj.cpu().numpy(), labels.cpu().numpy())
        edge_index = torch.tensor(hetero_adj).nonzero()
