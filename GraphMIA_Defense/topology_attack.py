...
class PGDAttack(BaseAttack):
    ...
    def attack(self, ori_features, ori_adj, labels, idx_attack, num_edges,
        ...
        for t in tqdm(range(epochs)):
            ...
            # TODO: 改动了这里
            output = victim_model(ori_features, adj_norm)[0]
            # TODO: add random noise to output
            # output = output.detach()
            # output += torch.randn_like(output) # (torch.randn_like(output) * stddev + mean)
            ...
            #     print("loss= {:.4f}".format(loss.item()), "test_accuracy= {:.3f}".format(best_test_acc))
